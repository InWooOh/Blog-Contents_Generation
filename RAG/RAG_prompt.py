...
def generate(Lecture_Type, Target_Audience, curriculum, API_key):
    ...
    vectorstore3 = Chroma.from_documents(documents=data_QnA, embedding=OpenAIEmbeddings(model="text-embedding-3-small"))
    ...
    qna_list_공통 = retriever.invoke("tag가 '공통' 인 문서들을 모두 찾아주세요.")   
    ...
    qna_list_비전공자 = retriever.invoke(f"tag가 '{Target_Audience}' 인 문서들을 모두 찾아주세요." )
    qna_list_특화 = retriever.invoke("tag가 '특화' 인 문서들을 모두 찾아주세요.")  
    ...
    prompt_template = PromptTemplate(
        ...
        - 이어서 그에 대한 세부사항은 불릿 형태로 상세하게 나열하세요.
    ...
    input_token = len(encoder.encode(prompt_template.template))
